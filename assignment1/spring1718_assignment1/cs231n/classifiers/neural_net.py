...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    z = np.dot(X, W1) + b1  # (N, hidden_size)
    ...
    softmax = scores / np.sum(scores, axis=1).reshape(N, 1)
    ...
    grads['W2'] = np.dot(h.T, dscores) / N      # (H, C)
    grads['b2'] = np.sum(dscores, axis=0) / N   # (C,)
    grads['W1'] = np.dot(X.T, dh) / N           # (D, H)
    grads['b1'] = np.sum(dh, axis=0) / N        # (H,)
